...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `https://www.googleapis.com/youtube/v3/playlists?part=snippet,contentDetails&mine=true&maxResults=${maxResults}${pageToken ? `&pageToken=${pageToken}` : ""}`;
    ...
    if (!res.ok) {
      return NextResponse.json({ error: "Falha ao buscar playlists" }, { status: res.status });
    }
